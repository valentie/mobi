...
- (void)viewDidLoad {
    ...
    [self.lblHeader setText:NSLocalizedString(@"Have an account?", @"Have an account?")];
    ...
    [self.btnForgotPassword setTitle:NSLocalizedString(@"Forgot Password?", @"Forgot Password?") forState:UIControlStateNormal];
    ...
    [self.btnForgotPassword.titleLabel setFont:[UIFont systemFontOfSize:FONTSIZE21]];
...
-(IBAction)btnLoginPressed:(id)sender
{
    ...
    self.flOperation = [self.flUploadEngine postDataToServer:postParams path:[NSString stringWithFormat:@"%@login",API_END_POINT]];
    [self.flOperation addCompletionHandler:^(MKNetworkOperation* operation) {
        ...
        if (jsonData.statusCode > 0)
        {
            ...
            [self alertStatus:NSLocalizedString(@"Sign in success.", @"Sign in success.") :NSLocalizedString(@"You are now signed in!", @"You are now signed in!") :1];
...
-(IBAction)btnFacebookLoginPressed:(id)sender
{
    ...
    [login logInWithReadPermissions:@[@"email"] handler:^(FBSDKLoginManagerLoginResult *result, NSError *error) {
        if (error) {
            // Process error
             NSLog(@" error");
            [self alertStatus:NSLocalizedString(@"Sign in Failed.", @"Sign in Failed.") :NSLocalizedString(@"Facebook Login Error!", @"Facebook Login Error!") :0];
            
        } else if (result.isCancelled) {
            // Handle cancellations
            NSLog(@" cancelled");
            [self alertStatus:NSLocalizedString(@"Sign in Failed.", @"Sign in Failed.") :NSLocalizedString(@"Cancelled by user", @"Cancelled by user") :0];
                    ...
                    [[[FBSDKGraphRequest alloc] initWithGraphPath:@"me" parameters:@{@"fields": @"id, name, email, gender"}] startWithCompletionHandler:^(FBSDKGraphRequestConnection *connection, id result, NSError *error)
                     {
                         if (!error)
                         {
                             ...
                             [self alertStatus:NSLocalizedString(@"Sign in Failed.", @"Sign in Failed.") :NSLocalizedString(@"Failed", @"Failed") :0];
                         }
                     }];
                }
...
{
    GSForgotPasswordViewController *detailViewController = [[GSForgotPasswordViewController alloc] initWithNibName:@"GSForgotPasswordViewController" bundle:nil];
    GSAppDelegate *appDelegate = (GSAppDelegate*)[[UIApplication sharedApplication] delegate];
...
-(void)textFieldDidBeginEditing:(UITextField *)textField
{
    ...
    [self.scrollView setContentOffset:CGPointMake(0, self.loginView.frame.origin.y-50) animated:YES];
}
