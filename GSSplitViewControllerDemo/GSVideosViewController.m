...
@interface GSVideosViewController ()

@end

@implementation GSVideosViewController
...
- (void)viewDidLoad {
    ...
    [navBarButtonSettings addTarget:self action:@selector(btnRightPressed:) forControlEvents:UIControlEventTouchUpInside];
    self.navigationItem.rightBarButtonItems = [[NSArray alloc] initWithObjects:navBarButtonSettingsItem, nil];
...
{
    UIActionSheet *popup = [[UIActionSheet alloc] initWithTitle:NSLocalizedString(@"Select Video", @"Select Video") delegate:self cancelButtonTitle:NSLocalizedString(@"Cancel", @"Cancel") destructiveButtonTitle:nil otherButtonTitles:
                            NSLocalizedString(@"Upload Video", @"Upload Video"),
                            nil];
    popup.tag = 1;
...
-(void)reloadData
{
    ...
    NSURL *aUrl = [NSURL URLWithString:[NSString stringWithFormat:@"http://%@/%@videos",API_HOST,API_END_POINT]];
    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:aUrl cachePolicy:NSURLRequestUseProtocolCachePolicy timeoutInterval:60.0];
    [request setHTTPMethod:@"POST"];
    
    NSString *user_id = [[NSUserDefaults standardUserDefaults] stringForKey:@"user_id"];
    NSString *postString = [NSString stringWithFormat:@"user_id=%@",user_id];
