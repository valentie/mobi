...
@implementation GSOnDemandEducationCollectionView
...
-(void)reloadData
{
    ...
    NSURL *aUrl = [NSURL URLWithString:[NSString stringWithFormat:@"http://%@/%@ondemand_education_sectioned2",API_HOST,API_END_POINT]];
    ...
    NSLog(@"searching for %@ in category %i perpage: %i and sortby %@", searchText, category_id, (int)PER_PAGE, sortBy);
...
{
    //NSLog(@"numberOfSectionsInCollectionView %i", [cellItems count]);
...
{
    //NSLog(@"numberOfItemsInSection %i", [[cellItems objectAtIndex:section] count]);
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)cv cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [cell.imageView setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@ondemand_education/screenshots/%i.png",API_IMAGE_HOST,[[video valueForKey:@"video_id"] intValue]]] placeholderImage:nil];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if([delegate respondsToSelector:@selector(onDemandPressed:)])
    {
        ...
        if (exclusive == 1 && purchased == 0)
        {
            ...
            // not logged in!
            [self alertStatus:@"Exclusive content is only available to our registered users. Please log in to continue." :@"Log In" :0];
...
{
    if ([self.delegate respondsToSelector:@selector(collectionCellDidPressCategoryForEducation:)]) {
        [delegate collectionCellDidPressCategoryForEducation:cat_id];
    }
}
